...
PhysicsOld::PhysicsOld(const Tmpl8::vec2& pos, float radius)
    : position{ pos }
...
const Tmpl8::vec2& PhysicsOld::getPos() const // Getter
{
    return position;
}

void PhysicsOld::setPos(const Tmpl8::vec2& pos) // Setter
{
    position = pos;
}

const Tmpl8::vec2& PhysicsOld::getVel() const
{
    return velocity;
}

void PhysicsOld::setVel(const Tmpl8::vec2& vel)
{
    velocity = vel;
}

float PhysicsOld::getRadius() const // Getter
{
    return radius;
}

void PhysicsOld::setRadius(float _radius)
{
    radius = _radius;
}

bool PhysicsOld::checkCollision(const PhysicsOld& other) const
{
...
void PhysicsOld::update(float deltaTime)
{
    position += velocity * deltaTime;
}

void PhysicsOld::draw(Tmpl8::Surface* surface)
{
    for (int i = 0; i < 64; i++)
    {
...
namespace Tmpl8
{
    ...
    // Draw balls
    PhysicsOld ball1{ {300,0}, 16 };
    PhysicsOld ball2{ {320, ScreenHeight - 50}, 49 };
    ...
    // Debug
    void DrawDebugInfo(Surface* screen, const PhysicsOld& ball1, const PhysicsOld& ball2)
    {
        char buffer[128];
